  ...
  const updateUserProfileData = async ()=>{
    try {
      ...
      const { data } = await axios.post(`${backendUrl}/api/user/update-profile`, formData, {
        headers: {
          Authorization: `Bearer ${token}`, // ✅ Fixed: Send token in proper format
        },
      });
      if (data.success) {
  ...
  return userData && (
...

    {
      isEdit ? <label htmlFor="image">
        ...
        <div className='mt-10'>
          {
            isEdit ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all cursor-pointer' onClick={updateUserProfileData}>Save Information</button> : <button className='border border-primary px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all cursor-pointer' onClick={()=>setIsEdit(true)}>Edit</button>
          }
        </div>
    </div>
  )
